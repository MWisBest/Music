...
public class MediaPlaybackActivity extends Activity implements MusicUtils.Defs,
	...
	private RepeatingImageButton.RepeatListener mRewListener =
			new RepeatingImageButton.RepeatListener() {
				public void onRepeat( View v, long howlong, int repcnt ) {
					// TODO: un-long
					scanBackward( repcnt, (int)howlong );
				}
			};
	...
	private RepeatingImageButton.RepeatListener mFfwdListener =
			new RepeatingImageButton.RepeatListener() {
				public void onRepeat( View v, long howlong, int repcnt ) {
					// TODO: un-long
					scanForward( repcnt, (int)howlong );
				}
			};
	...
	public boolean onOptionsItemSelected( MenuItem item ) {
		...
		try {
			switch( item.getItemId() ) {
							...
							.setPositiveButton( R.string.create_playlist_create_text, new DialogInterface.OnClickListener() {
								...
								public void onClick( DialogInterface dialog, int which ) {
									...
									if( name.length() > 0 ) {
										...
										int id = MusicUtils.idForPlaylist( MediaPlaybackActivity.this, name );
										Uri uri;
										if( id >= 0 ) {
											// overwrite existing playlist
											uri = ContentUris.withAppendedId( MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, id );
											MusicUtils.clearPlaylist( MediaPlaybackActivity.this, id );
										} else {
										...
										if( uri != null ) {
											...
											MusicUtils.addToPlaylist( MediaPlaybackActivity.this, list, playlist );
										}
									}
								}
							} );
					...
					input.addTextChangedListener( new TextWatcher() {
						...
						public void onTextChanged( CharSequence s, int start, int before, int count ) {
							...
							if( newText.trim().length() == 0 ) {
								...
								if( MusicUtils.idForPlaylist( MediaPlaybackActivity.this, newText ) >= 0 ) {
				...
				case DELETE_ITEM: {
					if( mService != null ) {
						String desc = getString( R.string.delete_song_desc, mService.getTrackName() );
						long[] list = new long[1];
						list[0] = MusicUtils.getCurrentAudioId();
				...
				case KeyEvent.KEYCODE_DPAD_LEFT:
					...
					if( mService != null ) {
						if( !mSeeking && mStartSeekPos >= 0 ) {
							...
							// TODO: un-long
							scanBackward( -1, (int)(event.getEventTime() - event.getDownTime()) );
							mPauseButton.requestFocus();
							mStartSeekPos = -1;
						}
					}
				...
				case KeyEvent.KEYCODE_DPAD_RIGHT:
					...
					if( mService != null ) {
						if( !mSeeking && mStartSeekPos >= 0 ) {
							...
							// TODO: un-long
							scanForward( -1, (int)(event.getEventTime() - event.getDownTime()) );
							mPauseButton.requestFocus();
							mStartSeekPos = -1;
						}
					}
...
*/
			...
			case KeyEvent.KEYCODE_DPAD_LEFT:
				...
				// TODO: un-long
				scanBackward( repcnt, (int)(event.getEventTime() - event.getDownTime()) );
				return true;
			case KeyEvent.KEYCODE_DPAD_RIGHT:
				if( !useDpadMusicControl() ) {
					break;
				...
				// TODO: un-long
				scanForward( repcnt, (int)(event.getEventTime() - event.getDownTime()) );
				return true;
	...
	private void scanBackward( int repcnt, int delta ) {
		if( mService == null ) return;
		try {
			if( repcnt == 0 ) {
				...
				int newpos = mStartSeekPos - delta;
				if( newpos < 0 ) {
	...
	private void scanForward( int repcnt, int delta ) {
		if( mService == null ) return;
		try {
			if( repcnt == 0 ) {
				mStartSeekPos = mService.position();
	...
	private int refreshNow() {
		if( mService == null )
			return 500;
		try {
			int pos = mPosOverride < 0 ? mService.position() : mPosOverride;
			if( (pos >= 0) && (mDuration > 0) ) {
				mCurrentTime.setText( MusicUtils.makeTimeString( this, pos / 1000 ) );
				int progress = 1000 * pos / mDuration;
			...
			int remaining = 1000 - (pos % 1000);

			// approximate how often we would need to refresh the slider to
			// move it smoothly
			int width = mProgress.getWidth();
			if( width == 0 ) width = 320;
			int smoothrefreshtime = mDuration / width;
