...
import me.mwisbest.music.MusicUtils.ServiceToken;
...
public class TrackBrowserActivity extends ListActivity
	...
	public boolean onContextItemSelected( MenuItem item ) {
		switch( item.getItemId() ) {
			...
			case DELETE_ITEM: {
				String desc = getString( R.string.delete_song_desc, mCurrentTrackName );
		...
		if( mGenre != null ) {
			...
			switch( mPlaylist ) {
				...
				case "recentlyadded":
					...
					Uri urira = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
					if( !TextUtils.isEmpty( filter ) ) {
						urira = urira.buildUpon().appendQueryParameter( "filter", Uri.encode( filter ) ).build();
					}
					int X = MusicUtils.getIntPref( this, "numweeks", 2 ) * (3600 * 24 * 7);
					where.append( " AND " + MediaStore.MediaColumns.DATE_ADDED + ">" );
					where.append( System.currentTimeMillis() / 1000 - X );
					ret = queryhandler.doQuery( urira,
							mCursorCols, where.toString(), null,
							MediaStore.Audio.Media.DEFAULT_SORT_ORDER, async );
					break;
				default:
					Uri urid = MediaStore.Audio.Playlists.Members.getContentUri( "external",
							Long.valueOf( mPlaylist ) );
					if( !TextUtils.isEmpty( filter ) ) {
						urid = urid.buildUpon().appendQueryParameter( "filter", Uri.encode( filter ) ).build();
					}
					mSortOrder = MediaStore.Audio.Playlists.Members.DEFAULT_SORT_ORDER;
					ret = queryhandler.doQuery( urid, mPlaylistMemberCols,
