...
public class ArtistAlbumBrowserActivity extends ExpandableListActivity
	...
	private static final String LOG_TAG = "ArtistAlbumBrowser";
		...
		switch( item.getItemId() ) {
			...
			case DELETE_ITEM: {
				...
				if( mCurrentArtistId != null ) {
					...
					desc = getString( R.string.delete_artist_desc, mCurrentArtistName );
				} else {
					list = MusicUtils.getSongListForAlbum( this, Long.parseLong( mCurrentAlbumId ) );
					desc = getString( R.string.delete_album_desc, mCurrentAlbumName );
				}
	...
	static class ArtistAlbumListAdapter extends SimpleCursorTreeAdapter implements SectionIndexer {
		...
		ArtistAlbumListAdapter( Context context, ArtistAlbumBrowserActivity currentactivity,
			...
			mDefaultAlbumIcon = (BitmapDrawable)r.getDrawable( R.drawable.albumart_mp_unknown_list, null );
		...
		@Override
		public void bindGroupView( @NonNull View view, Context context, @NonNull Cursor cursor, boolean isexpanded ) {
		...
		@Override
		public void bindChildView( @NonNull View view, Context context, @NonNull Cursor cursor, boolean islast ) {
			...
			if( unknown || art == null || art.length() == 0 ) {
				iv.setBackground( mDefaultAlbumIcon );
